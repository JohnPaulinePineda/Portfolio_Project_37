...
|      **[A.2]** To ensure that both under-extraction and over-extraction are assessed, models with 3 and 4 and factors were sequentially evaluated for their interpretability and theoretical meaningfulness.
|      **[A.3]** The choice of 4 factors was supported by the **Beta** and **Bartlett** methods for determining the optimal number of factors.
|
...
|             **[B.5.6]** Standardized Cronbach's Alpha = 0.88
...
|             **[B.6.3]** Standardized Cronbach's Alpha = 0.89
...
|             **[B.7.3]** Standardized Cronbach's Alpha = 0.85
|
...
|             **[C.5.3]** Standardized Cronbach's Alpha = 0.89
...
|             **[C.7.3]** Standardized Cronbach's Alpha = 0.85
...
|             **[C.8.4]** Standardized Cronbach's Alpha = 0.86
|
|
...
|             **[B.6.4]** Standardized Cronbach's Alpha = 0.89
|      **[B.7]** Factor 3 was a latent variable with higher loading towards the following descriptors:
|             **[B.7.1]** <span style="color: #FF0000">MEUNDA</span>: Loading = 1.05, Communality = 1.06
|             **[B.7.2]** Standardized Cronbach's Alpha = 0.85
|
| **[C]** Results for the exploratory factor analysis using a **4-Factor Structure** were as follows:
|      **[C.1]** Standardized Root Mean Square of the Residual = 0.01
|      **[C.2]** Tucker-Lewis Fit Index = 1.08
...
|      **[A.1]** 4 factors would be sufficient for an optimal balance between comprehensiveness and parsimony. 
|      **[A.2]** To ensure that both under-extraction and over-extraction are assessed, models with 3 and 4 and factors were sequentially evaluated for their interpretability and theoretical meaningfulness.
|      **[A.3]** The choice of 4 factors was supported by the **Beta**, **Bartlett** and **Adjusted BIC** methods for determining the optimal number of factors.
|
| **[B]** Results for the exploratory factor analysis using a **3-Factor Structure** were as follows:
|      **[B.1]** Standardized Root Mean Square of the Residual = 0.03
...
CFA_4F <- 'CARDIO =~ HDMORT + STMORT + OVWINC
           PULMON =~ SMPREV + PDMORT
           MENTAL =~ MEUNDA + ARTINC
           NEPHRO =~ RDMORT + DBMORT'
...
fitMeasures(CFA_4F_MODEL, c("cfi", "srmr"))
...
standardizedSolution(CFA_4F_MODEL)
...
CFA_3F <- 'CARDIO =~ HDMORT + STMORT + OVWINC + RDMORT + DBMORT
           PULMON =~ SMPREV + PDMORT
           MENTAL =~ MEUNDA + ARTINC'
...
fitMeasures(CFA_3F_MODEL, c("chisq.scaled", "df", "pvalue.scaled"))

fitMeasures(CFA_3F_MODEL, c("rmsea.scaled", "rmsea.ci.lower.scaled", "rmsea.ci.upper.scaled", "rmsea.pvalue.scaled"))

fitMeasures(CFA_3F_MODEL, c("cfi.scaled", "srmr"))

residuals(CFA_3F_MODEL)$cov

parameterEstimates(CFA_3F_MODEL, standardized=FALSE) %>% 
  filter(op == "=~") %>% 
  select(Item=rhs, Standardized=est, ci.lower, ci.upper, SE=se, Z=z, 'p-value'=pvalue)

parameterEstimates(CFA_3F_MODEL, standardized=FALSE, rsquare = TRUE) %>% 
  filter(op == "r2") %>% 
  select(Item=rhs, R2 = est) 

semTools::compRelSEM(CFA_3F_MODEL, tau.eq=T, obs.var=T, return.total=T)

CFA_3F_MODEL <- cfa(CFA_3F, data = DPA.Descriptors.Numeric)
